"""
pages/1_🛡️_Cyber_Portal.py
--------------------------
Cyber Portal — Authority Identity & OTP Dispatcher (Streamlit Multi-Page).
Generates secure one-time credentials for Action Routing & Delivery Audit.
"""
...
# ── Page Configuration ────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Cyber Portal — Authority OTP Dispatcher",
    page_icon="🛡️",
    layout="wide",
...
st.markdown(
    ...
    <div style="background: linear-gradient(90deg, #0d1b2a, #1b263b); padding: 16px 22px; border-radius: 10px; border-left: 6px solid #00cc88; margin-bottom: 20px;">
        <h2 style="color: #ffffff; margin: 0; font-size: 1.45rem; display: flex; align-items: center; gap: 10px;">
            🛡️ CYBER PORTAL &nbsp;<span style="font-size: 0.85rem; background: #00cc88; color: black; font-weight: bold; padding: 2px 10px; border-radius: 15px;">AUTHORITY CONSOLE</span>
        </h2>
        <p style="color: #cbd5e1; margin: 5px 0 0 0; font-size: 0.90rem;">
            <strong>Authority Identity & OTP Dispatcher:</strong> Generates secure one-time credentials for Action Routing & Delivery Audit.
        </p>
    </div>
    """,
...
with st.sidebar:
    ...
    chosen_officer_key = st.radio(
        ...
        key="sidebar_officer_radio_page",
    ...
    st.caption("👈 Use the navigation links at the top of this sidebar to switch back to the **Analyst & Intelligence Map**.")
    ...
    def confirm_clear_dialog():
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Yes, Clear Data", type="primary", use_container_width=True, key="dlg_clear_yes_page"):
                res = clear_all_data()
                st.session_state.last_generated_otp = None
                st.session_state.last_otp_user = None
                ...
                st.rerun()
        with c2:
            if st.button("No, Cancel", type="secondary", use_container_width=True, key="dlg_clear_cancel_page"):
                st.rerun()

    if st.button("🗑️ Clear Data (Purge TXT & CSV)", type="secondary", use_container_width=True, key="btn_clear_data_page"):
...
with tab_otp:
    ...
    st.caption("Generate a one-time 6-digit OTP for this authoritative user to unlock Action Routing decision panels and Delivery Audit.")
    ...
    if is_online:
        ...
        st.success(
            ...
            "This user can unlock Action Routing in the Analyst Dashboard using this OTP."
        )

        st.info("📌 **Single-User Rule Enforced:** No more than one user can be logged into the same username at the same time.")

        if st.button("🚪 Force Sign Out Active Session", key=f"force_logout_page_{cur_user['username']}"):
        ...
        if (
            ...
            st.markdown(
                ...
                <div style="background: #0f2b1d; border: 2px solid #00cc88; padding: 18px; border-radius: 8px; margin: 15px 0;">
                    ...
                    <div style="color: #cbd5e1; font-size: 0.85rem;">
                        👉 Copy this OTP and paste it into the <b>Analyst Dashboard (Tab 4: Delivery Model & Action Routing)</b> to unlock enforcement actions.
                    </div>
                </div>
                """,
                unsafe_allow_html=True,
            ...
            if st.button("↩️ Return to Directory View", key="return_dir_page_btn", type="primary"):
                st.session_state.last_generated_otp = None
                st.session_state.last_otp_user = None
                st.rerun()

        else:
            if st.button(f"⚡ Generate OTP for {cur_user['name']}", type="primary", key=f"gen_otp_page_{cur_user['username']}"):
                try:
...
with tab_actions:
    ...
    if not user_actions:
        st.info(f"No actions committed yet by {cur_user['name']}. Perform actions in Action Routing and commit to view them here.")
    else:
